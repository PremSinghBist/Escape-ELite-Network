...
import DataAnalyzer
MODEL_PATH = "model/pretrain_models/feature_learning_model/bilstm_256-04.hdf5"
...
INTEGRATED_MODEL_PATH = "model/pretrain_models/discriminator-model/integrated_model"
BASE_ANALYSIS_PATH = "model/pretrain_models/discriminator-model/integrated_model/analysis"
...
def plot_greany_test_predictions(targets, y_pred):
    ...
    #Save result to CSV | Flatten used for numpy becuse shape comes as 2d ie : shape (n, 1)
    df = pd.DataFrame({
        'target': targets.flatten(),
        'predicted' :  y_pred.flatten()
    })
    save_path = BASE_ANALYSIS_PATH+'greany_predictions.csv'
