...
GSAID_SIG_SPIKE_PATH = "/home/perm/sars_escape_netv2/data/gen/GISAIDX_SIG.fa"
GSAIDX_RAW_SEQ_PATH = "/home/perm/sars_escape_netv2/data/raw/spikeprot0730.fasta"
...
def split_gsaid_sequences():
      #SEG.getGsaid_raw_seq_path()

    seqs  = SEG.read_sequences(GSAIDX_RAW_SEQ_PATH)
    print("Total Sequences: ",len(seqs))
    unique_seqs = remove_duplicate_sequences(seqs)
...
def generate_gsaid_sequences(sig_seq_name, non_sig_seqs_name):
    print(f"Proceeding to generate sig seq {sig_seq_name} and non sig: {non_sig_seqs_name} seqs")
...
def read_window_file_using_column_name(file_path, column_name):
    df = pd.read_csv(file_path)
...
def generate_gisaid_sig_windows_sarsx():
    segment_length = 20
    sig_path = GSAID_SIG_SPIKE_PATH #SEG.getGsaid_significantSeqs_sample_path()
    print('**started reading sequences from path: ',sig_path)
    ...
    for seq in significant_seqs:
        ...
        for mutant, position in muts_dic.items():
            ...
            if len(window) != segment_length:
                continue
            if window.__contains__(AMBIGIOUS_CHAR):
                ambigious_count += 1
                continue
            dict_data = {'window_seqs': window, 'mutant': mutant }
            escape_seq_data.append(dict_data)
    
    print("Total Escaped Ambigious Char X windows :",ambigious_count)
    #Create dataframe from 
    df = pd.DataFrame(escape_seq_data)
    print("Total GISAID Significant windows Before duplicate removed: ", len(df))
    #duplicate_removed_windows = list (set (windows))
    df = df.drop_duplicates() #remove duplicate windows 
    print("Total GISAID Cleansed Significant windows: ", len(df))
    file_name = "/home/perm/sars_escape_netv2/data/gen/significantx.csv"
...
def construct_window_sequence_sarsx(file_path, window_size=20):
    ...
    print(f'''Total Uniques segments: {len(segments)} 
          Total Segments including duplicates: {segments_including_duplicates} 
          Untranslated windows founds: {untranslated_residue_counter} ''' )
    return segments
...
if __name__ == "__main__":
    ...
    #generate_gsaid_sequences('GISAIDX_SIG.fa', 'GISAIDX_NON_SIG.fa')   
    generate_gisaid_sig_windows_sarsx()
    ...
    #updated new code
    # generate_gisaid_sig_windows_with_pos()
