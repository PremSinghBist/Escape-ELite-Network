...
def generate_gisaid_sig_windows_with_pos():
    '''
    Newly added code to get window with position information
    '''
    segment_length = 20
    sig_path = SEG.getGsaid_significantSeqs_sample_path()
    significant_seqs = SEG.read_sequences(sig_path)
    ...
    for seq in significant_seqs:
        ...
        if not muts_dic:
            continue
        for mutant, position in muts_dic.items():
            window, start_pos, end_pos = SEG.extract_window_with_pos(segment_length=segment_length, pos=position, input_seq = seq)
            if len(window) != segment_length:
                continue
            #already the particular sequences exists , so do not add
            if window in window_dict:
                continue
            #duplicates automatically removed while trying to be added in dictionary 
            #if window already avaiable in dict , do not add 
            if window in window_dict :
                continue
                
            window_dict[window] = [start_pos, end_pos, seq]
            
    file_name = "data/gen/sig_windows_with_pos_and_full_seq.csv"
    seqs, sp, ep, full_seqs = [], [], [], []
    for sequence, values in window_dict.items():
        seqs.append(sequence)
        sp.append(values[0])
        ep.append(values[1])
        full_seqs.append(values[2])
    
    df  = pd.DataFrame({'window' : seqs, 'start_pos': sp, 'end_pos': ep, 'full_seq':full_seqs })
    df.to_csv(file_name, index=False) #Do not write index 0, 1, 2,3 for each row
    print(f"GISAID file with path {file_name} generated successfully")
